...
[YTrain, ~, UTrain] = runMultiLayer(XTrain, W0, V0);
[YTest, ~, UTest]  = runMultiLayer(XTest , W0, V0);
ErrTrain(1) = sum(sum((YTrain - DTrain).^2)) / (NTrain * NClasses);
ErrTest(1)  = sum(sum((YTest  - DTest ).^2)) / (NTest  * NClasses);

for n = 1:numIterations
    % Add your own code here
    grad_v = (2/NTrain) * (YTrain - DTrain)' * UTrain; % Gradient for the output layer
    grad_w = (2/NTrain) * (Vout' * (YTrain - DTrain)').*(1-UTrain.^2)' * XTrain; % And the input layer
    ...
    % Evaluate errors
    [YTrain, ~, UTrain] = runMultiLayer(XTrain, Wout, Vout);
